...
#     # Function to fetch image URLs based on the product ID
#     def fetch_posters(product_id):
#         # Filter the dataset to get the corresponding image URLs
#         filtered_dataset = dataset[dataset['ID'] == product_id]
        
#         # Check if the DataFrame is empty
#         if filtered_dataset.empty:
#             return []  # Return empty list if no images are found

#         # Return the list of image URLs (after stripping spaces and special characters)
#         img_urls = filtered_dataset['imgURL'].values[0].split('|')
#         cleaned_img_urls = [url.strip() for url in img_urls]  # Remove trailing/leading spaces
#         return cleaned_img_urls
...
#         # Fetch top 6 recommended products
#         for i in distances[1:7]:  # Skipping the first one (it will be the selected product itself)
#             recommended_product_names.append(products.iloc[i[0]]['Name'])
#             product_id = products.iloc[i[0]]['ID']
#             posters = fetch_posters(product_id)
            
#             # Append the first valid poster URL (or a placeholder if no valid URLs)
#             if posters:
#                 recommended_product_posters.append(posters[0])  # Use only the first URL
#             else:
#                 recommended_product_posters.append("https://via.placeholder.com/150")  # Placeholder image
...
#                         cols[col].image("https://via.placeholder.com/150", caption="Error loading image", width=150)
#                 else:
#                     cols[col].empty()  # Leave empty space if fewer than 6 recommendations
